        ...
        classMetatable = {
            ...
            __call = function(classPrototype, ...)
                ...
                constructRecursively = function(classPrototype, ...)
                    if not classPrototype then
                        return
                    end
                    constructRecursively(rawget(classPrototype, baseKeyword))
                    --- 先Clone所有类型模板的字段给实例
                    if type(classPrototype[fieldsKeyword]) == "table" then
                        for fieldName, fieldValue in pairs(classPrototype[fieldsKeyword]) do
                            if fieldName ~= constructorKeyword and fieldName ~= baseKeyword then
                                DeepCopy(classPrototype[fieldsKeyword], obj)
                            end
                        end
                    end
                    --- 然后用构造函数初始化
