...
缺点：需要相同的mesh和shader（但可以使用MaterialPropertyBlock来应用不同的属性）
...
- 不能使用多pass。如果使用其他Tag `"LightMode" = "SRPDefault"`来支持多pass，则每个pass都会产生一个draw call，且多个（使用了该shader的）物体之间都无法合批。比如2个pass，2个物体，FrameDebugger里仍会显示4个SRP Batch
